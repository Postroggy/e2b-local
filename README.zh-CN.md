...
SBX 使用本机安装的 Docker Sandboxes 控制面，不会把 `e2b-infra` 作为
仓库代码或运行时依赖带进来。先从当前公开源码构建唯一需要的镜像：
...
构建会解析公开 `e2b-dev/infra` 的当前 revision，只 sparse checkout
`packages/envd` 与 `packages/shared`，然后编译 `envd`。最终镜像只有
`envd`、`sbx-init` 和 `sbx-tunnel`，不包含上游源码树，也不依赖
`e2b-infra` 运行。

默认要求先执行 `sbx login`，并通过已认证的 `sandboxd` 管理生命周期。
只有明确需要在未登录时使用本地 Docker fallback，才设置
`allow_degraded: true`。

SBX 支持 create/pause/resume/delete、gateway 重启恢复、volume、logs、
metrics、Docker hijack PTY，以及 guest `envd` 的反向隧道。Snapshot 目前
明确不支持：现有 Docker Sandboxes 没有可用的 image commit/export 路径，
gateway 会返回 `501`，不会把不完整 fallback 冒充成 snapshot。
