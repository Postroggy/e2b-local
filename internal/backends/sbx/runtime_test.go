...
func TestSbxLifecycleModeRequiresLoginByDefault(t *testing.T) {
	...
	_, err := runtime.lifecycleMode(context.Background())
	if err == nil || !strings.Contains(err.Error(), "sbx login") {
		t.Fatalf("expected default login error mentioning sbx login, got %v", err)
	}

	runtime.cfg.AllowDegraded = true
	runtime.loginKnown = false
	mode, err := runtime.lifecycleMode(context.Background())
	if err != nil {
		t.Fatalf("allow degraded: %v", err)
	}
	if mode != sbxModeDegraded {
		t.Fatalf("expected degraded mode, got %q", mode)
	}
}

func TestSbxLifecycleModeUsesSandboxdWhenLoggedIn(t *testing.T) {
	runtime := &SbxRuntime{
		cfg:            gateway.DefaultConfig().Sbx,
	...
	mode, err := runtime.lifecycleMode(context.Background())
	if err != nil {
		t.Fatalf("resolve lifecycle mode: %v", err)
	}
	if mode != sbxModeSandboxd {
		t.Fatalf("expected sandboxd mode, got %q", mode)
	}
}

func TestSbxRuntimeEnvironmentIncludesRuntimeContract(t *testing.T) {
	runtime := &SbxRuntime{cfg: gateway.SbxRuntimeConfig{EnvdPort: 50123}}
	...
	}, gateway.SbxTemplateConfig{StartCmd: "echo ready"}, sbxModeSandboxd,
	...
	if env[sbxManagedEnv] != "true" || env[sbxModeEnv] != string(sbxModeSandboxd) {
...
func TestSbxRuntimeInfoFromContainerRestoresTunnelPorts(t *testing.T) {
	runtime := &SbxRuntime{cfg: gateway.SbxRuntimeConfig{EnvdPort: 49983, TunnelPublicHost: "127.0.0.1"}}
	...
	info, err := runtime.runtimeInfoFromContainer(gateway.SandboxRuntimeInfo{}, dockertypes.ContainerJSON{
		...
		Config: &container.Config{Env: []string{
			...
			sbxModeEnv + "=" + string(sbxModeSandboxd),
...
func TestSbxRuntimeStatePersistsBootstrapContractPrivately(t *testing.T) {
	...
	}, sbxModeSandboxd, map[string]string{
...
func TestDialLongUnixSocketRestoresWorkingDirectory(t *testing.T) {
	...
	if err := os.Chdir(directory); err != nil {
		t.Fatalf("enter socket directory: %v", err)
	}
	listener, err := net.Listen("unix", "metrics.sock")
	if restoreErr := os.Chdir(currentDirectory); restoreErr != nil {
		t.Fatalf("restore working directory after listener setup: %v", restoreErr)
	}
	if err != nil {
