...
// sbxRuntimeState preserves the values sandboxd intentionally does not put in
// the generated container configuration. It is required to restart envd and
// reverse tunnels after sandboxd's native stop/start lifecycle.
...
func newSbxRuntimeState(req SandboxRuntimeCreateRequest, info SandboxRuntimeInfo, mode sbxMode, environment map[string]string, tunnels []TunnelSpec) sbxRuntimeState {
...
func (r *SbxRuntime) loadRuntimeState(sandboxID string) (sbxRuntimeState, bool, error) {
	...
	if err != nil {
		return sbxRuntimeState{}, false, err
	}
