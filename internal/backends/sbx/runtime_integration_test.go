	...
	t.Cleanup(func() {
		if !deleted {
			_ = runtime.DeleteSandbox(context.Background(), gateway.SandboxRuntimeInfo{
				...
				ContainerName: cfg.Sbx.ContainerNamePrefix + sandbox.SandboxID,
				MachineID:     cfg.Sbx.ContainerNamePrefix + sandbox.SandboxID,
			})
		}
	})
...
func newSbxIntegrationRuntime(t *testing.T, ctx context.Context) *SbxRuntime {
	...
	if _, err := os.Stat(cfg.SandboxdSocket); err != nil {
	...
	for _, template := range templates {
		if template.TemplateID == "sbx" && template.ImageRef == cfg.DefaultImage {
			return runtime
		}
	}
	t.Skipf("sbx image %q is unavailable; run scripts/build-sbx-image.sh", cfg.DefaultImage)
	return nil
}
